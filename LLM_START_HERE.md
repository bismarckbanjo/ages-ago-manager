...
| `lib/googleFields.ts` | **Single source of truth for the Google / Merchant Center metafields** (`mm-google-shopping`). Each field's change key, filter label, metafield key, level, type, options. Includes `google_product_category` (legacy `string` type — `metafieldsSet` writes it fine), `brand`, `product_type`; **all fields are product-level**. Imported by the filter UI, changes UI, matcher, and execute route — add a new Google field here only. |
